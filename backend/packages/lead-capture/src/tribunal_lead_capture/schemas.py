"""Pydantic request/response schemas for the ``lead-capture`` block.

Combines the lead-magnet schemas (``app.schemas.lead_magnet``) and the lead-source
schemas (``app.schemas.lead_source``). The host re-exports these via back-compat
shims so existing ``from app.schemas.lead_magnet import ...`` /
``from app.schemas.lead_source import ...`` imports keep working.
"""
