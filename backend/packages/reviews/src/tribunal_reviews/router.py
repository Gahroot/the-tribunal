"""HTTP surface for the ``reviews`` block.

Two routers:

* ``get_router()`` — authenticated, workspace-scoped operator dashboard +
  settings, mounted at ``/workspaces/{workspace_id}/reviews``.
* ``get_public_router()`` — the no-auth rating landing page that implements the
  negative-feedback firewall (the rating gate), mounted at ``/p/reviews``.

Both prefixes + tags are baked into the routers so the host mounts them
prefix-free (``app.include_router(get_router())``). Core DI primitives come only
through ``app.core_api``; the AI reply drafter is the block's own helper.
"""
...
from app.core_api import DB, CurrentUser, get_workspace
from app.models.workspace import Workspace

from .ai import generate_review_reply
from .schemas import (
...
def get_router() -> APIRouter:
    """Return the authenticated, workspace-scoped reviews router."""
    ...
    async def update_review_settings(
        ...
        return await ReviewService(db).update_settings(
            workspace, update.model_dump(exclude_unset=True)
        )
...
def get_public_router() -> APIRouter:
    """Return the no-auth public rating-gate landing-page router."""
    public_router = APIRouter(prefix="/p/reviews", tags=["Public Reviews"])
