"""HTTP surface for the ``offers`` block.

Two routers:

* ``get_router()`` — authenticated, workspace-scoped offer authoring (AI generate,
  list/create/get/update/delete, get-with-lead-magnets, attach/detach/reorder lead
  magnets) mounted at ``/workspaces/{workspace_id}/offers``.
* ``get_public_router()`` — the no-auth public offer opt-in pages mounted at
  ``/p/offers``. This preserves the exact public URLs ``/api/v1/p/offers/{slug}``
  and ``/api/v1/p/offers/{slug}/opt-in`` that the public ``app/p/offers`` pages
  hit — do not change these prefixes.

Both prefixes + tags are baked into the routers so the host mounts them prefix-free
(``app.include_router(get_router())``). Core DI primitives come only through
``app.core_api``; cross-block calls go through the sibling blocks' public APIs:

* **lead-capture** — ``deliver_lead_magnet_to_lead``, ``LeadMagnet``,
  ``LeadMagnetLead``, ``LeadMagnetResponse`` (declared dependency).
* **agent-brain** — ``generate_offer_content`` (declared dependency).
* **contacts** — the shared ``Contact`` model (declared dependency).
"""
...
# Authenticated, workspace-scoped offer authoring routes.
offers_router = APIRouter()

# Public, no-auth offer opt-in pages.
public_router = APIRouter()


@offers_router.post("/generate", response_model=GeneratedOfferContent)
...
@offers_router.get("", response_model=PaginatedOffers)
...
@offers_router.post("", response_model=OfferResponse, status_code=status.HTTP_201_CREATED)
...
@offers_router.put("/{offer_id}", response_model=OfferResponse)
...
@offers_router.get("/{offer_id}/with-lead-magnets", response_model=OfferResponseWithLeadMagnets)
...
@offers_router.post("/{offer_id}/lead-magnets", response_model=OfferResponseWithLeadMagnets)
...
@offers_router.delete(
    "/{offer_id}/lead-magnets/{lead_magnet_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
@offers_router.put("/{offer_id}/lead-magnets/reorder", response_model=OfferResponseWithLeadMagnets)
...
def get_router() -> APIRouter:
    """Return the authenticated, workspace-scoped offers router.

    Mounted at ``/workspaces/{workspace_id}/offers``. The prefix + tags are baked
    in so the host mounts it prefix-free.
    """
    router = APIRouter()
...
def get_public_router() -> APIRouter:
    """Return the no-auth public offer opt-in router (mounted at ``/p/offers``).

    Preserves the exact public URLs ``/api/v1/p/offers/{slug}`` and
    ``/api/v1/p/offers/{slug}/opt-in`` that the public ``app/p/offers`` pages hit.
    """
    router = APIRouter()
    router.include_router(
        public_router,
