"""Redis-backed rate limiting for the public embed widget block.

The embed endpoints are unauthenticated and call out to expensive third-party
services (OpenAI Realtime, OpenAI Chat, Telnyx). This module provides per-scope
rolling-window rate limits keyed by ``(scope, identifier)`` so we can apply
distinct caps per IP and per agent public_id without coupling the routes to
Redis details.

Vendored into the widget block so the block depends only on the core surface
(``app.core`` / ``app.db``) and not on the compliance block's rate limiter. The
atomic INCR-with-limit Lua script is inlined here for the same reason; it mirrors
the shared ``app.services.rate_limiting.rate_limiter`` pattern.
"""
...
# Atomic check-and-increment with a per-key TTL. Vendored from the shared
# rate-limiter so the block carries no sideways import into the compliance block.
