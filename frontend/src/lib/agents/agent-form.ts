...
export const editAgentFormSchema = z.object({
  ...
  bookingDepositMode: z.enum(["off", "card", "20", "50", "experiment"]),
  isActive: z.boolean(),
  ...toolsFields,
  ...ivrFields,
  ...reminderCoreFields,
...
export function buildUpdateAgentRequest(data: EditAgentFormValues): UpdateAgentRequest {
  return {
    ...
    booking_deposit_mode: data.bookingDepositMode,
...
export function agentToEditFormValues(agent: Agent): EditAgentFormValues {
  return {
    ...
    bookingDepositMode:
      (["off", "card", "20", "50", "experiment"] as const).find(
        (mode) => mode === agent.booking_deposit_mode,
      ) ?? "off",
