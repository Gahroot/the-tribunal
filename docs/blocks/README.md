...
- [`lead-capture`](./lead-capture/BLOCK.md) — tier `A`, status `manifest`, extraction effort `high`; integrates `telnyx`, `resend`.
...
- [`reviews`](./reviews/BLOCK.md) — tier `A`, status `manifest`, extraction effort `medium`; integrates `telnyx`.
- [`short-links`](./short-links/BLOCK.md) — tier `A`, status `manifest`, extraction effort `low`.
- [`voice`](./voice/BLOCK.md) — tier `B`, status `manifest`, extraction effort `high`; integrates `openai`, `elevenlabs`, `telnyx`, `mac-relay`.
- [`widget`](./widget/BLOCK.md) — tier `A`, status `manifest`, extraction effort `medium`; integrates `openai`, `telnyx`.
